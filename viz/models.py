...
def prepare(df, prv, data_test):
    ...
    df = df.sort_values(by = ["Province","Year","Month","Day","Time"],ignore_index=True)
    ...
    ##
    data = convert_category2numeric(data, ['Time','Weather',"Month","Year","Day"])
    data = pd.concat((data, pd.get_dummies(data['Label'],drop_first=True)), axis=1)
...
def train(train_set, X_colums, Y_colums):
    ...
    # building the model and fitting the data
        ...
        if max(log_reg.pvalues.to_list()) > 0.05:
            X_colums.remove(max(log_reg.pvalues.to_dict(), key=log_reg.pvalues.to_dict().get))
        else:
            done = False
    return log_reg
