...
def parse_all_province(year):
    ...
    for file in os.listdir(path):
        if (
            file.endswith(".csv")
            and file != "weather_data_[2020].csv"
            and file != "worldweatheronline2021.csv"
            and "VietNam" not in file
        ):
            if first:
                ...
                data["Dew point"] = (
                    data["Dew point"]
                    .astype(str)
                    .apply(lambda x: x.strip("°C"))
                    .astype(float)
                )
                data["Humidity"] = data["Humidity"].fillna(
                    data["Humidity"].mean()
                )
                data["Dew point"] = data["Dew point"].fillna(
                    data["Dew point"].mean()
                )
                ...
                tmp["Humidity"] = tmp["Humidity"].fillna(
                    tmp["Humidity"].mean()
                )
                tmp["Dew point"] = tmp["Dew point"].fillna(
                    tmp["Dew point"].mean()
                )
...
def process_merged_data(merged_df):
    ...
    merged_df["Humidity"] = (
        merged_df["Humidity"].apply(lambda x: x.strip("%")).astype(float)
    )
    merged_df["Dew point"] = (
        merged_df["Dew point"].apply(lambda x: x.strip("°C")).astype(float)
    )
    merged_df["Rain"] = (
        merged_df["Rain"].apply(lambda x: x.strip("mm")).astype(float)
    )
    merged_df["Cloud"] = (
        merged_df["Cloud"].apply(lambda x: x.strip("%")).astype(float)
    )
    merged_df["Pressure"] = (
        merged_df["Pressure"].apply(lambda x: x.strip("mb")).astype(float)
    )
    merged_df["Wind"] = (
        merged_df["Wind"].apply(lambda x: x.strip("km/h")).astype(float)
    )
    merged_df["Gust"] = (
        merged_df["Gust"].apply(lambda x: x.strip("km/h")).astype(float)
    )
    merged_df["Dir"] = (
        merged_df["Dir"].apply(lambda x: x.strip("deg")).astype(float)
    )
...
def process_data(df, save=False):
    ...
    if save:
        df.to_csv(
            "../../data/vietnam/vietnam_[2017-2022]_fix.csv", index=False
        )
    else:
        return df
