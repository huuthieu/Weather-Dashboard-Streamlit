...
with col2:
    ...
    option2 = st.selectbox("Choose month", tuple(list_months))

with col3:
    # st.subheader("Choose feature")
    option3 = st.selectbox("Choose feature", tuple(list_features[:-1]))
...
with st.spinner("Wait for it..."):
    ...
    x = (
        data.groupby(["Name", "Month"])
        .mean()[option3]
        .reset_index(level=[0, 1])
    )
    if option3 == "Rain":
        x[option3] = x[option3] * 24
    high = np.ceil(1.2 * x[option3].max())
    ...
    p.patches(
        ...
        },  # plot_data là field dùng để  quy định độ đậm nhạt màu tô
        line_color="black",
        line_width=0.25,
        fill_alpha=1,
    )
