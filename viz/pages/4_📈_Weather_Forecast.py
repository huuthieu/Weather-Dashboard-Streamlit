...
from cfg import *
from utils.format import rounding_time
from utils.get_data import get_data, get_data_msn
...
option = st.selectbox("Choose province", tuple(list_provinces))
...
with st.spinner("Wait for it..."):
    temperature = (
        soup.find_all("div", {"class": "weather-widget-temperature"})[0]
        .find_all("p")[0]
        .text.strip()
    )
    # wind_speed = soup.find_all("span", {"class": "wind-speed"})[0].text
    weather = (
        soup.find_all("div", {"class": "weather-widget-icon"})[0]
        .find_all("p")[0]
        .text.strip()
    )
